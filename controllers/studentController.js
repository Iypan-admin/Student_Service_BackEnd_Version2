...
const registerStudent = async (req, res) => {
    const { registration_number, name, state, center, email, password, phone, is_referred, referred_by_center, affiliated_link } = req.body;

    if (!name || !state || !email || !password || !phone) {
        return res.status(400).json({ error: 'All fields are required' });
    }

    // Validate phone number: exactly 10-digit Indian mobile numbers (starts with 6-9)
    const phoneRegex = /^[6-9]\d{9}$/;
    if (!phoneRegex.test(phone)) {
        return res.status(400).json({ error: 'Please enter a valid 10-digit Indian mobile number' });
    }

    // Fetch state details to check if selected mode is Online
    ...
    if (stateErr || !stateData) {
        return res.status(400).json({ error: 'Invalid state/mode selected' });
    }

    const isOnline = stateData.state_name.toLowerCase() === 'online';
    const isOffline = stateData.state_name.toLowerCase() === 'offline';

    // Center is required for non-offline modes
    if (!isOffline && !center) {
        return res.status(400).json({ error: 'Center is required' });
    }

    // Enforce Referral Centre is mandatory when Online mode is selected
    if (isOnline && !referred_by_center) {
        return res.status(400).json({ error: 'Referral center is required when Online mode is selected' });
    }
    ...
    const studentData = {
        ...
        referred_by_center: referred_by_center || null,
